  ...
  <div>
    ...
    >
      ...
      <el-table-column label="操作">
        <template slot-scope="{row}">
          ...
          <HintButton
            :disabled="row.level===4 || $hasBP('btn.Permission.add')  === false"
            type="primary"
            icon="el-icon-plus"
            size="mini"
          ...
          <HintButton
            ...
            icon="el-icon-edit"
            size="mini"
            :disabled="row.level===1 || $hasBP('btn.Permission.update')  === false"
            @click="toUpdatePermission(row)"
            :title="row.level===4 ? '修改功能' : '修改菜单'"
          />

          <HintButton
            :disabled="row.level===1 || $hasBP('btn.Permission.remove')  === false"
            type="danger"
            icon="el-icon-delete"
            size="mini"
    ...
    <el-dialog
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="addOrUpdatePermission" :disabled="$hasBP('btn.Permission.add')  === false">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
