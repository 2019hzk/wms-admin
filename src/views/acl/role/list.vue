<template>
  <div>
    ...
    <el-card class="operate-container" shadow="never">
      <el-form label-width="70px" size="small">
        <el-form-item label="角色名称">
          <el-input style="width: 100%" v-model="tempSearchObj.roleName" placeholder="角色名称"></el-input>
        </el-form-item>
    ...
    <el-card class="operate-container" shadow="never">
      ...
      <el-button class="btn-add" size="mini" @click="addRole" :disabled="$hasBP('btn.Role.add')  === false">添加</el-button>
      <el-button class="btn-add" size="mini" @click="removeRoles()" :disabled="$hasBP('btn.Role.remove')  === false">批量删除</el-button>
    </el-card>
      ...
      <el-table-column label="操作" width="200" align="center">
        <template slot-scope="{row}">
          <HintButton size="mini" type="info" icon="el-icon-info" title="分配权限"
            @click="$router.push(`/acl/role/auth/${row.id}?roleName=${row.roleName}`)" :disabled="$hasBP('btn.Role.assgin')  === false"/>

          <HintButton size="mini" type="primary" icon="el-icon-check" title="确定"
            @click="updateRole(row)" v-if="row.edit" :disabled="$hasBP('btn.Role.update')  === false"/>
          ...
          <HintButton size="mini" type="danger" icon="el-icon-delete" title="删除角色"
            @click="removeRole(row)" :disabled="$hasBP('btn.Role.remove')  === false"/>
        </template>
      </el-table-column>
    </el-table>
