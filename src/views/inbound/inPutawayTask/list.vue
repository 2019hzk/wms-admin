<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column prop="expectCount" label="预期到货数量" width="120px"/>
      <el-table-column prop="actualCount" label="实际数量" width="110px"/>
      <el-table-column prop="unqualifiedCount" label="不合格数量" width="120px"/>
      <el-table-column prop="putawayCount" label="上架数量" width="110px"/>
      <el-table-column prop="statusName" label="状态" width="110px"/>
      <el-table-column prop="createTime" label="创建时间" width="160px"/>
      <el-table-column label="操作" width="100" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button type="text" v-if="scope.row.status == 'PENDING_APPROVEL'"size="mini" @click="putaway(scope.row)">上架</el-button>
<!--          <el-button type="text" v-if="scope.row.status == 'PENDING_INVENTORY'"size="mini" @click="syncInventory(scope.row.id)" :disabled="$hasBP('btn.inPutawayTask.sync')  === false">同步库存</el-button>-->
    ...
    <el-dialog title="上架" :visible.sync="dialogVisible" width="90%" >
      ...
      <div style="margin-top:20px;font-weight:bold;">收货记录</div>
      <el-table
        v-loading="listLoading"
      ...
      <el-table
        ...
        <el-table-column label="指定库位" width="220px;">
          <template slot-scope="scope">
            <el-cascader
        ...
        <el-table-column label="上架数量" >
          <template slot-scope="scope">
            {{ scope.row.actualCount - scope.row.unqualifiedCount }}
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-form ref="flashPromotionForm" size="small" style="margin-top: 20px;">
        <el-form-item label="上架备注">
          <el-input v-model="inPutawayFormVo.remarks"/>
        </el-form-item>
      </el-form>

      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisible = false" size="small">取 消</el-button>
        <el-button type="primary" @click="submitPutaway()" size="small" :disabled="$hasBP('btn.inPutawayTask.putaway')  === false">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
<script>
import api from '@/api/inbound/inPutawayTask'
...
export default {
  ...
  methods: {
    ...
    putaway(row) {
      ...
      this.inPutawayFormVo.id = row.id

      inOrderApi.show(row.inOrderId).then(response => {
        that.inOrder = response.data.inOrder
        that.inApproveTaskList = response.data.inApproveTaskList
        that.inReceivingTaskList = response.data.inReceivingTaskList
        that.inOrder.inOrderItemList.forEach(function(row) {
          row.wareIds = [that.inOrder.warehouseId]
          row.unqualifiedCount = 0
        })

        that.wareList.forEach(function(item, index) {
          if (item.value != that.inOrder.warehouseId) {
            that.wareList.splice(index,1);
          }
        })
      })
    },
    submitPutaway() {
      var that = this
      that.inPutawayFormVo.inPutawayItemVoList = []
      that.inOrder.inOrderItemList.forEach(function(row) {
        if(row.wareIds[0].length < 4) {
          that.$message.info('请选择库位')
          return
        }
        that.inPutawayFormVo.inPutawayItemVoList.push({
          goodsId: row.goodsId,
          actualCount: row.actualCount,
          unqualifiedCount: row.unqualifiedCount,
          putawayCount: row.actualCount - row.unqualifiedCount,
          warehouseId: row.wareIds[0],
          storeareaId: row.wareIds[1],
          storeshelfId: row.wareIds[2],
          storehouseId: row.wareIds[3],
        })
      })

      api.putaway(that.inPutawayFormVo).then(response => {
