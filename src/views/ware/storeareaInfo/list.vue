<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column prop="areaTypeName" label="库区类型" />
      ...
      <el-table-column label="操作" width="150" align="center" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button type="text" size="mini" @click="removeDataById(scope.row.id)" :disabled="$hasBP('btn.storeareaInfo.remove')  === false">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="添加/修改" :visible.sync="dialogVisible" width="40%" >
      <el-form ref="flashPromotionForm" label-width="150px" size="small" style="padding-right: 40px;">
        <el-form-item label="仓库">
          <el-select
            v-model="storeareaInfo.warehouseId"
            placeholder="请选择仓库">
            <el-option
              v-for="item in warehouseInfoList"
              :key="item.id"
        ...
        <el-form-item label="名称">
          <el-input v-model="storeareaInfo.name"/>
        </el-form-item>
        <el-form-item label="库区类型">
          <el-select
            v-model="storeareaInfo.areaTypeId"
            placeholder="请选择库区类型">
            <el-option
        ...
        <el-form-item label="面积(平方米)">
          <el-input v-model="storeareaInfo.builtArea"/>
        </el-form-item>
        <el-form-item label="管理人">
          <el-autocomplete
            v-model="storeareaInfo.manageName"
        ...
        <el-form-item label="状态">
          <el-radio-group v-model="storeareaInfo.status">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="saveOrUpdate()" size="small" :disabled="$hasBP('btn.storeareaInfo.add')  === false || $hasBP('btn.storeareaInfo.update')  === false">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
<script>
import api from '@/api/base/storeareaInfo'
...
export default {
  ...
  created() {
    ...
    dictApi.findByDictCode('AreaType').then(response => {
  ...
  methods: {
    ...
    add(){
      ...
      this.storeareaInfo = Object.assign({}, defaultForm)
    },
    ...
    fetchDataById(id) {
      api.getById(id).then(response => {
        this.storeareaInfo = response.data
      })
    },

    saveOrUpdate() {
      this.saveBtnDisabled = true // 防止表单重复提交
      if (!this.storeareaInfo.id) {
        this.saveData()
      } else {
        this.updateData()
      }
    ...
    // 新增
    saveData() {
      api.save(this.storeareaInfo).then(response => {
        if (response.code) {
          this.$message({
            type: 'success',
    ...
    // 根据id更新记录
    updateData() {
      api.updateById(this.storeareaInfo).then(response => {
        if (response.code) {
          this.$message({
            type: 'success',
            message: response.message
    ...
    handleSelect(item) {
      debugger
      this.storeareaInfo.manageId = item.id
      this.storeareaInfo.manageName = item.name
      this.storeareaInfo.managePhone = item.phone
    }
  }
}
</script>
